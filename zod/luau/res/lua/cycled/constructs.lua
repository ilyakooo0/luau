...
local operand = {3,100,5,- 10,- 5,1e4,- 1e4}
local operator = {"+","-","*","/","//","%","^","&","|","^","<<",">>","==","~=","<",">","<=",">="}
...
local basiccases = {{"nil",nil},{"false",false},{"true",true},{"10",10},{"(0==_ENV.GLOB1)",0 == _ENV.GLOB1}}
local prog = 
if _ENV.GLOB1 == 0 then
basiccases[2][1]="F"
prog="    local F <const> = false
    if %s then IX = true end
    return %s
"
else
basiccases[4][1]="k10"
prog="    local k10 <const> = 10
    if %s then IX = true end
    return %s
  "
end
print("testing short-circuit optimizations (" .. _ENV.GLOB1 .. ")")
local binops <const> = {{" and ",(a,b)
if not a then
return a
else
return b
