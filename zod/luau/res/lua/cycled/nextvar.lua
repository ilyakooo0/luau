...
assert(a.x == 1)
local x = 0
for k,v in ipairs({10,20,30,["x"] = 12})
do
x=x + 1
assert(k == x and v == x * 10)
end
for _ in ipairs({["x"] = 12,["y"] = 24})
do
assert(nil)
end
x=false
...
check(math,0,mp2(s))
local sizes = {0,1,2,3,4,5,7,8,9,15,16,17,30,31,32,33,34,254,255,256,500,1e3}
...
for n,v in pairs(a)
do
if not package.loaded[n] and type(v) ~= "function" and not string.find(n,"^[%u_]") then
_G[n]=undef
end
collectgarbage()
end
...
assert(table.maxn({[1e3] = true}) == 1e3)
...
assert(a[# a])
print("+")
local a = {[1] = 1,[1.1] = 2,["x"] = 3,[string.rep("x",1e3)] = 4,[print] = 5,[checkerror] = 6,[coroutine.running()] = 7,[true] = 8,[io.stdin] = 9,[{}] = 10}
local b = {}
;
for i = 1, 10
do
...
assert(next(b) == nil)
local t = {[{1}] = 1,[{2}] = 2,[string.rep("x ",4)] = 3,[100.3] = 4,[4] = 5}
local n = 0
for k,v in pairs(t)
do
n=n + 1
...
local co = coroutine.wrap((t)
for k,v in pairs(t)
do
local k1 = next(t)
assert(k == k1)
t[k]=nil
local expected = (type(k) == "table" and k[1] or type(k) == "function" and k() or string.sub(k,1,1))
assert(expected == v)
coroutine.yield(v)
end
end)
...
local t = {}
local proxy = setmetatable({},{["__len"] = ()
return # t
end,["__index"] = t,["__newindex"] = t})
test(proxy,t)
local count = 0
t=setmetatable({},{["__newindex"] = (t,k,v)
count=count + 1
;
rawset(t,k,v)
end})
...
debug.setmetatable(u2,{["__newindex"] = (_,k,v)
tab2[k]=v
end})
...
assert(next(a,nil) == 1e3 and next(a,1e3) == nil)
assert(next({}) == nil)
assert(next({},nil) == nil)
for a,b in pairs({})
do
...
assert(i == a.n)
local t = setmetatable({10,20,30},{["__pairs"] = (t)
