...
local res = T.testC("    # push 16 elements on the stack
...
checkmessage("  local a = setmetatable({}, {__add = 34})
  a = a + 1
","metamethod 'add'")
checkmessage("  local a = setmetatable({}, {__lt = {}})
  a = a > a
","metamethod 'lt'")
...
_G.D=debug
checkmessage("  -- create light udata
...
local a,b = 1, {
  ...
  {d = x and aaa[x or y]}}
","global 'aaa'")
checkmessage("local x,y = {},1
if math.sin(1) == 0 then return 3 end    -- return
x.a()","field 'a'")
checkmessage("prefix = nil
insert = nil
while 1 do
  local a
...
local res,msg = load(s)
assert(not res and (string.find(msg,"too many") or string.find(msg,"overflow")))
end
...
local a,b = load(s)
assert(c > 255 and string.find(b,"too many upvalues") and string.find(b,"line 5"))
s="
function foo ()
  local "
for j = 1, 300
