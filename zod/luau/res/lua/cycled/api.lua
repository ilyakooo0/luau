...
tcheck(t,{10,20,30,40})
T.testC("  warningC "#This shold be a"
  warningC " single "
  warning "warning"
  warningC "#This should be "
  warning "another one"
")
local f = T.makeCfunc("    getglobal error
    pushstring bola
...
t=pack(T.testC("call 2,-1; return *",(a,b)
return 1,2,3,4,a,b
end,"alo","joao"))
...
assert(T.testC("pushnum 10; pushnum 3; arith ^; return 1") == 1e3)
...
assert(T.testC("pushstring 10; pushstring 3; arith ^; return 1") == 1e3)
assert(T.testC("arith /; return 1",2,0) == 10 / 0)
a=T.testC("pushnum 10; pushint 3; arith \; return 1")
assert(a == 3 and math.type(a) == "float")
...
assert(a == true and b == 10)
local t = setmetatable({["x"] = 20},{["__len"] = (t)
return t.x
end})
a,b,c=T.testC("   len 2;
   Llen 2;
...
a=setmetatable({["x"] = "u"},{["__concat"] = (a,b)
return a.x .. "." .. b.x
end})
x,y=T.testC("  pushnum 5
  pushvalue 2;
  pushvalue 2;
  concat 2;
  pushvalue -2;
...
T.totalmem(T.totalmem() + 1e4)
assert(T.checkpanic("newuserdata 20000") == MEMERRMSG)
T.totalmem(0)
if not _soft then
local msg = T.checkpanic("      pushstring "function f() f() end"
      loadstring -1; call 0 0
      getglobal f; call 0 0
    ")
assert(string.find(msg,"stack overflow"))
end
assert(T.checkpanic("    pushstring "return {__close = function () Y = 'ho'; end}"
    newtable
    loadstring -2
    call 0 1
    setmetatable -2
...
local s = string.rep("pushnil;checkstack 1 XX;",1e6)
checkerr("overflow",T.testC,s)
collectgarbage("restart")
print("+")
end
local lim = _soft and 500 or 1.2e4
...
assert(next(t) == nil)
prog,g,t=nil
a=T.testC("  loadstring 2; pcall 0 1 0;
...
getmetatable(b).__newindex=(t,i,v)
a[i]=v
end
...
local a = T.testC("    getfield 2 _012345678901234567890123456789012345678901234567890123456789
    return 1
  ",t)
assert(a == 32)
local a = T.testC("    pushnum 33
...
print("+")
_G.TT={}
T.sethook("  # set a line hook after 3 count hooks
  sethook 4 0 '
    getglobal TT;
...
assert(a == nil and c == 3 and type(b) == "string")
T.loadlib(L1)
a,b,c=T.doremote(L1,"  string = require'string'
...
()
local x = setmetatable({10},{["__close"] = (y)
...
local a,b = T.testC("    call 0 1   # create resource
    pushnil
...
local a = {T.testC("    call 0 1   # create resource
    toclose 2 # mark it to be closed
    return 2
  ",newresource)}
assert(type(a[1]) == "string" and a[2][1] == 11)
assert(# openresource == 0)
local a,b = pcall(T.makeCfunc("    call 0 1   # create resource
    toclose -1 # mark it to be closed
    error       # resource is the error object
  "),newresource)
assert(a == false and b[1] == 11)
assert(# openresource == 0)
local a,b = pcall(T.makeCfunc("    newtable   # create non-closable object
    toclose -1 # mark it to be closed (should raise an error)
    abort  # will not be executed
  "))
assert(a == false and string.find(b,"non%-closable value"))
local function check
(n)
assert(# openresource == n)
end
_ENV.xxx=true
local a = T.testC("    pushvalue 2  # stack: S, NR, CH, NR
...
local a = T.testC("    pushvalue 2  # stack: S, NR, CH, NR
  ...
  ",newresource,check)
assert(a == 3)
local a,b = pcall(T.makeCfunc("    pushint 32
    toclose -1
  "))
...
b=testamem("doing nothing",()
return 10
end)
assert(b == 10)
testamem("state creation",()
local st = T.newstate()
if st then
T.closestate(st)
end
...
testamem("empty-table creation",()
return {}
end)
testamem("string creation",()
return "XXX" .. "YYY"
end)
testamem("coroutine creation",()
return coroutine.create(print)
end)
testamem("to-be-closed variables",()
local flag = 
local x <close> = setmetatable({},{["__close"] = ()
flag=true
end})
flag=false
local x = {}
...
;
G=nil
testamem("running code on new thread",()
return T.doonnewstack("local x=1") == 0
end)
testamem("loadstring",()
return load("x=1")
end)
local testprog = "local function foo () return end
local t = {"x"}
AA = "aaa"
...
assert(_G.AA == "aaax")
testamem("gsub",()
local a,b = string.gsub("alo alo","(a)",(x)
return x .. "b"
end)
return (a == "ablo ablo")
end)
testamem("dump/undump",()
...
testamem("using coroutines",()
local a = coroutine.wrap(()
...
testamem("auxiliary buffer",()
return (# table.concat(a,",") == 20 * lim + lim - 1)
end)
testamem("growing stack",()
local function foo
(n)
if n == 0 then
return 1
...
local L = T.newstate()
T.loadlib(L)
local res = (T.doremote(L,"    _ENV = require"_G"
    local T = require"T"
    local a = {}
