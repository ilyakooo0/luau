...
if rawget(_G,"T") then
collectgarbage("stop")
local a = {[{}] = 4,[3] = 0,["alo"] = 1,["a1234567890123456789012345678901234567890"] = 10}
...
x=foo("hi")
;
assert(mt.A == "hi" and A == 1e3)
assert(x("*") == mt.A .. "*")
local _ENV = {["assert"] = assert,["A"] = 10}
;
local _ENV = {["assert"] = assert,["A"] = 20}
;
assert(A == 20)
;
x=A
...
checkro("z","    local a, z <const>, b = 10;
    function foo() a = 20; z = 32; end
  ")
checkro("var1","    local a, var1 <const> = 10;
...
local a = {}
local b <close> = false
local x <close> = setmetatable({"x"},{["__close"] = (self)
a[# a + 1]=self[1]
end})
local w,y <close>,z = func2close((self,err)
assert(err == nil)
;
a[# a + 1]="y"
end,10,20)
...
local function foo
()
local x <close> = func2close((self,msg)
assert(string.find(msg,"@y"))
error("@x")
end)
local x1 <close> = func2close((self,msg)
assert(string.find(msg,"@y"))
end)
local gc <close> = func2close(()
collectgarbage()
end)
local y <close> = func2close((self,msg)
assert(string.find(msg,"@z"))
error("@y")
end)
local z <close> = func2close((self,msg)
assert(msg == nil)
error("@z")
end)
return 200
...
local x123 <close> = func2close((_,msg)
assert(msg == nil)
error("@X")
end)
os.exit(false)
...
assert(not stat and string.find(msg,"variable 'x' got a non%-closable value"))
local function foo
()
...
local a3 <close> = func2close((_,msg)
assert(msg == nil)
error(123)
end)
...
local function foo
()
local x0 <close> = func2close((_,msg)
assert(msg == 202)
track[# track + 1]="x0"
end)
local x <close> = func2close(()
...
assert(track[1] == "foo" and track[2] == "x" and track[3] == "xx" and track[4] == "x0")
...
local closemsg = 
local close = func2close((self,msg)
T.alloccount()
closemsg=msg
end)
...
local function test
()
local xxx <close> = func2close((self,msg)
assert(msg == "not enough memory")
;
error(1e3)
end)
local xx <close> = func2close((self,msg)
assert(msg == "not enough memory")
;
end)
local x <close> = enter(0)
...
local _,msg = pcall(test)
assert(msg == 1e3 and closemsg == "not enough memory")
collectgarbage()
local s = string.rep("a",1e4)
local m = T.totalmem()
collectgarbage("stop")
s=string.upper(s)
assert(T.totalmem() - m <= 1.1e4)
...
local x = func2close((_,msg)
trace[# trace + 1]="x"
end)
local y = func2close((_,msg)
trace[# trace + 1]="y"
end)
debug.sethook(hook,"r")
local t = {T.testC("       toclose 2      # x
       pushnum 10
       pushint 20
       toclose 3      # y
       return 2
    ",x,y)}
debug.sethook()
checktable(trace,{"return sethook","y","return ?","x","return ?","return testC"})
checktable(t,{10,20})
end
local trace = {}
local function hook
...
checktable(trace,{"return sethook","return close","x","return close","return foo"})
...
local extra <close> = func2close((self)
...
local y = false
local co = coroutine.wrap(()
local xv <close> = func2close(()
x=true
end)
local yv <close> = func2close(()
y=true
end)
coroutine.yield(100)
coroutine.yield(200)
...
local y = 0
co=coroutine.wrap(()
local xx <close> = func2close((_,err)
y=y + 1
;
...
local token = 0
local count = 0
local f = T.makeCfunc("      toclose 1
      toclose 2
      return .
    ")
local obj = func2close((_,msg)
count=count + 1
